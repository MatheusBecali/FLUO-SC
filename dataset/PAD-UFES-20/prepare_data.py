...
# Use "CandNC", "CarcinomasVsOthers", "CarcinomasVsCeratoses", "CarcinomaVsACK", "MelVsSek", "MelVsNev", "MelVsNevSek"
_type = 'CandNC'

# Or use a list
# _types = ["CandNC", "CarcinomasVsOthers", "CarcinomasVsCeratoses", "CarcinomaVsACK", "MelVsSek", "MelVsNev", "MelVsNevSek"]
...
# for _type in _types:
data_csv = pd.read_csv(os.path.join(BASE_PATH, f"metadata_{_type}.csv")).fillna("EMPTY")
new_cli_cols = list()
for c in clin_feats:
...
data_test.to_csv(os.path.join(BASE_PATH, f"pad-ufes-20_parsed_{_type}_test.csv"), index=False)
label_categorical_to_number (os.path.join(BASE_PATH, f"pad-ufes-20_parsed_{_type}_test.csv"), "diagnostic",
                            col_target_number="diagnostic_number",
                            save_path=os.path.join(BASE_PATH, f"pad-ufes-20_parsed_{_type}_test.csv"))
...
label_categorical_to_number (data_train, "diagnostic", col_target_number="diagnostic_number",
                            save_path=os.path.join(BASE_PATH, f"pad-ufes-20_parsed_{_type}_folders.csv"))
